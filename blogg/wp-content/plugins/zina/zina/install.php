...
function zina_install_database($force = false) {
	...
	if ($zc['database']) {
		if (!zdbq("SELECT 1 FROM {dirs} LIMIT 0")) {
			...
			if (zdbq("INSERT {dirs} (parent_id, path, level) VALUES (0, '.', 0)")) {
				zdbq("INSERT {system} (id, version) VALUES (1, '".ZINA_VERSION."')");
				return true;
			} else {
				zina_set_message(zt('Could not insert default value.'), 'warn');
			}
		} else {
