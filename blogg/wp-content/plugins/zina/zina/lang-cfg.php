...
$titles = array(
	'genre_hierarchy'=> zt('Custom Genre Hierarchy'),
	...
	'session_pls'=> zt('Allow user to have session playlist'),
	'asx'=> zt('Return .asx instead of .m3u playlists'),
	'sp_len'=> zt('Max length of session playlist'),
	...
	'stream_extinf'=> zt('Add #EXTINF Artist - Title info to playlists'),
	'stream_extinf_limit'=> zt('Limit #EXTINF ID3 references (performance)'),
	...
	'cat_split'=> zt('Split category page into different pages'),
	...
	'rss'=> zt('Generate podcasts for directories with files in them'),
	'rss_file'=> zt('Name of podcast/rss file'),
	'rss_cache'=> zt('Cache the generated Podcasts in the cache directory'),
	...
	'search_limit'=> zt('Limit search results to'),
	'search_images'=> zt('Show images in search results and live search'),
);

$subs = array(
	'db_search'=> zt('Requires a fully populated database.'),
	'search_limit'=> zt('Applies only to non-database search'),
	'rating_random_opts'=> zt('>= list, comma separated'),
	'genre_hierarchy'=> zt('Requires database. After enabling, check Functions above.'),
	...
	'cat_file'=> zt('Can include xml to overide settings per directory. e.g. @xml', array('@xml'=>'<category> <images>1</images> </category>')).' '.
		zt('Override options include images, columns, truncate, split [0,1,2], per_page, sort'),
	'cmp_cache'=> zt('Will take up lots of disk space, but less server processing.'),
	'cache_tmpl'=> zt('Caches main pages: Category/Artist/Albums for anonymous users.'),
	...
	'debug' => zt('Show more details error messages, etc.'),
...
$cats = array(
	...
	'integration' => array(
		't'=> zt('Last.fm / Twitter Integration'),
	...
	'genres' => array(
		't'=> zt('Genres'),
		'd'=> array(
			zt('Requires ID3 and Main Cache.'),
	...
	'pos' => array(
		...
		'd'=> array(
			...
			zt('<b>Windows/WinAmp</b> (You will most likely need http://www.jukkis.net/bgrun/bgrun.exe)').'<br/>'.
				zt('Command example').': <code>bgrun.exe C:\Progra~1\Winamp\winamp.exe $tmpfname.m3u >NUL</code>',
			zt('<b>Linux/mpg123</b><br/>').
				zt('Command example').': <code>/usr/bin/mpg123 -q --list $tmpfname.m3u 0 1> /dev/null 2>&1 &</code>',
			zt('<b>OS/2</b> and <b>eCS</b> try using Z! http://dink.org/z/').'<br/>'.
				zt('Command example').': <code>z.exe -p $tmpfname.m3u >NUL</code>'
			),
		),
	'other_media' => array(
		't'=> zt('Media Types'),
