...
function felsyncpost( $post_id ) {
	...
	$post_text = felprepposttext( $post->post_content, get_post_meta( $post_id, 'bbsync_truncate', true ) );
...
function felprepposttext( $post_text, $set_truncate = 0 ) {
	$opshuns = get_option('bbsync');

	if( $opshuns['nomore'] ) {
		$ptvers = get_extended( $post_text );
		$post_text = $ptvers['main'];
	} elseif( $opshuns['pageonly'] ) {
		list( $post_text, $throwaway ) = explode('<!--nextpage-->', $post_text, 2 );
	} elseif( ( isset( $_POST['truncate'] ) && $_POST['truncate'] === 0 ) || ( !( isset( $_POST['truncate'] ) && $_POST['truncate'] === 0 ) && $opshuns['truncate'] ) ) {
	//if POSTed truncate is 0 XOR default truncate is 0, no modification
	} elseif( $_POST['truncate'] ) {
	//if POSTed truncate is not 0
		$post_text = substr( $post_text, 0, $_POST['truncate'] ) . '...';
	...
	//if no POSTed truncate and special data for this 'un
		$post_text = substr( $post_text, 0, $set_truncate ) . '...';
	} elseif( $opshuns['truncate'] ) {
	//otherwise, go to default if exists
		$post_text = substr( $post_text, 0, $opshuns['truncate'] ) . '...';
	}

	if( !empty( $opshuns['link'] ) && $post_text != $post_text ) {
